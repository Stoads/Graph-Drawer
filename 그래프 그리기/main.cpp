...
typedef struct equation{
    int index;
    char* equate,leftPart[100000],rightPart[100000];
...
typedef struct virtualEquation{
    int index;
    nd* Tree;
    char equate[100000];
    struct virtualEquation *next;
}viEquat;

inEquat *equationList=(inEquat*)malloc(sizeof(inEquat));
inEquat *headList=equationList;
viEquat *virtualList=(viEquat*)malloc(sizeof(inEquat));
viEquat *headVirtual=virtualList;
int graph[500][500];
...
int createVirutal(viEquat* vIE){
    int cnt=2;
    while(vIE->next!=NULL){
        vIE=vIE->next;
        cnt++;
    }
    vIE->next=(viEquat*)malloc(sizeof(viEquat));
    vIE=vIE->next;
    *vIE={cnt,NULL,NULL,NULL};
    vIE=headVirtual;
    return 1;
}
/** \brief 조건식 추가
...
void deleteVirutal(viEquat* vIE,int index){
    if(vIE->index==index){
        if(vIE->next==NULL)return;
        viEquat* deleteIE;
        deleteIE=vIE;
        vIE=vIE->next;
        free(deleteIE);
        ...
        while(vIE->next->index!=index){
            vIE=vIE->next;
        }
        viEquat* deleteIE;
        deleteIE=vIE->next;
        vIE->next=vIE->next->next;
        vIE=vIE->next;
        free(deleteIE);
    }
    while(vIE!=NULL){
...
void changeVirutal(viEquat* vIE,int index,char* str){
    while(vIE->index!=index)
        vIE=vIE->next;
    vIE->Tree=(nd*)malloc(sizeof(nd));
    strcpy(vIE->equate,str);
    buildTree(str);
    *vIE->Tree=returnTree();
}
...
void printVirutalList(viEquat* vIE,int index,HDC hdc){
    HFONT hfont=CreateFont(18,0,0,0,5,0,0,0,ANSI_CHARSET,0,0,0,0,TEXT("Consolas"));
    SelectObject(hdc,hfont);
    while(vIE->index!=index){
        vIE=vIE->next;
    ...
    char dump[100000]={};
    sprintf(dump,"f%d(x)=",index);
    strcat(dump,vIE->equate);
    TextOut(hdc,0,605,dump,strlen(dump));
    DeleteObject(hfont);
}
/** \brief 그래프 그리기
...
void printUserListInterface(HWND hwnd,inEquat* pIE,int selected,int userPage,char *clr,int isItSel){
    HDC hdc=GetDC(hwnd);
    HFONT hfont=CreateFont(18,0,0,0,5,0,0,0,ANSI_CHARSET,0,0,0,0,TEXT("Consolas"));
    ...
    while(pIE!=NULL&&userPage-(pIE->index-1)/48==0){
        if(selected==pIE->index&&isItSel==1){
            SetTextColor(hdc,RGB(255,255,255));
            SelectObject(hdc,hBlackBrush);
            int tempcolor=pIE->color;
            //printf("%#x\n",tempcolor);
            for(int i=5;i>=0;i--){
                clr[i]=tempcolor%16;
                tempcolor/=16;
                if(clr[i]<10)clr[i]+=0x30;
                else clr[i]+=55;
            }
            //printf("%s",clr);
        }
...
void printVirtualListInterface(HWND hwnd,viEquat* vIE,int selected,int virutalPage,int isItSel){
//    HDC hdc=GetDC(hwnd);
//    HFONT hfont=CreateFont(18,0,0,0,5,0,0,0,ANSI_CHARSET,0,0,0,0,TEXT("Consolas"));
//    HBRUSH hBlackBrush=CreateSolidBrush(RGB(0,0,0));
//    HBRUSH hWhiteBrush=CreateSolidBrush(RGB(255,255,255));
//    SelectObject(hdc,hfont);
//    SetBkMode(hdc,TRANSPARENT);
//    while(vIE!=NULL&&virutalPage-(vIE->index-1)/48>0){
//        vIE=vIE->next;
//    }
//    while(vIE!=NULL&&virutalPage-(vIE->index-1)/48==0){
//        if(selected==vIE->index&&isItSel==2){
//            SetTextColor(hdc,RGB(255,255,255));
//            SelectObject(hdc,hBlackBrush);
//        }
//        else{
...
//            SelectObject(hdc,hWhiteBrush);
//        }
//            Rectangle(hdc,(vIE->index-1-virutalPage*48)%4*50+700,(vIE->index-1-virutalPage*48)/4*30+20,
//                      ((vIE->index-1-virutalPage*48)%4+1)*50+700,((vIE->index-1-virutalPage*48)/4)*30+50);
//        char dump[10];
//        sprintf(dump,"f %2d",vIE->index);
//        TextOut(hdc,(vIE->index-1-virutalPage*48)%4*50+710,((vIE->index-1-virutalPage*48)/4)*30+27,dump,strlen(dump));
//        vIE=vIE->next;
//    }
//    DeleteObject(hBlackBrush);
//    DeleteObject(hWhiteBrush);
...
nd sendTree(int index){
    viEquat* vIE=(viEquat*)malloc(sizeof(viEquat));
    *vIE=*virtualList;
    while(vIE->index!=index){
        vIE=vIE->next;
    }
...
LRESULT CALLBACK windporc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:{
        ...
        SetTextColor(hdc,RGB(0,0,0));
        printUserListInterface(hwnd,equationList,nowListp,userPage,colorRGB,whatSelected);
        printVirtualListInterface(hwnd,virtualList,nowVirtp,virtPage,whatSelected);
        for(int i=0;i<6;i++){
        ...
        if(whatSelected==2){
            printVirutalList(virtualList,nowVirtp,hdc);
        }
        DeleteObject(hfont);
        DeleteObject(hBlackBrush);
        DeleteObject(hWhiteBrush);
    ...
    case WM_LBUTTONDOWN:{
        ...
        if(isItCal)break;
        ...
        else if(x<=700&&y>380&&y<400){/**< 아래쪽 리스트 보기 */
            whatSelected=1;
        ...
        else if(x<=700&&y>=20&&y<=380){/**< 실질적 그래프 식 선택 */
            ...
            int i=(x-700)/50,j=(y-20)/30;
            if(j*4+i+1>checkVirutal(virtualList))break;
            whatSelected=2;
            nowVirtp=virtPage*48+j*4+i+1;
            RECT rect={700,20,900,400};
            InvalidateRect(hwnd,&rect,false);
            rect={0,600,500,630};
            InvalidateRect(hwnd,&rect,false);
        }
        else if(x>=450&&y<=600&&y>=570){/**< 입력식 리스트 안에 넣기 */
            int size=strlen(str);
            char dump[100000]={};
            strcpy(dump,str);
            if(size<=0)break;
            ...
            if(whatSelected==1)
                ...
                changeVirutal(virtualList,nowVirtp,dump);
    ...
    case WM_LBUTTONUP:{
        ...
        if(x>=500&&y>=400){
            ...
            case 20:{
                toggleTriangleFunction(' ','h');
                RECT rect={634,443,835,529};
                InvalidateRect(hwnd,&rect,false);
                break;
            }
            case 21:{
                toggleTriangleFunction('a','h');
                RECT rect={634,443,835,529};
                InvalidateRect(hwnd,&rect,false);
                break;
            }
        ...
        if(GetAsyncKeyState(VK_CONTROL)<0){
            if(GetAsyncKeyState('N')&0x0001){
                if(GetAsyncKeyState(VK_SHIFT)<0){
                    whatSelected=2;
                    createVirutal(virtualList);
                    nowVirtp=checkVirutal(virtualList);
                    virtPage=(nowVirtp-1)/48;
                    RECT rect={500,20,900,400};
                    InvalidateRect(hwnd,&rect,false);
                    rect={0,600,500,630};
                    InvalidateRect(hwnd,&rect,false);
                }
            ...
            if(GetAsyncKeyState(0x30)&0x0001){
                ...
                insertStr(hwnd,str,"+");
                break;
            }if(GetAsyncKeyState(0xBE)&0x0001){
                insertStr(hwnd,str,">");
                break;
            }if(GetAsyncKeyState(0xBC)&0x0001){
                insertStr(hwnd,str,"<");
                break;
            }if(GetAsyncKeyState(VK_DELETE)&0x0001){
                deleteVirutal(virtualList,nowVirtp);
                int temp=checkVirutal(virtualList);
                if(temp<nowVirtp)nowVirtp=temp;
                virtPage=(nowVirtp-1)/48;
                RECT rect={700,20,900,400};
                InvalidateRect(hwnd,&rect,false);
                rect={0,600,500,630};
